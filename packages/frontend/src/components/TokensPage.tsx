  ...
  return (
    <div className="max-w-4xl mx-auto space-y-16 py-8">
      <div className="flex flex-col md:flex-row items-end justify-between gap-6 px-4">
        <div>
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white tracking-tight">Créditos</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Gerencie seu saldo e desbloqueie recursos.</p>
        </div>
        ...
        <div className="flex items-center gap-6 pb-2">
          ...
          <button className="h-12 px-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full font-bold text-sm hover:scale-105 active:scale-95 transition-all shadow-lg shadow-slate-200 dark:shadow-none">
            Resgatar
          </button>
        </div>
      </div>
            ...
            {packages.map((pkg) => {
              ...
              return (
                <div key={pkg.id} className="flex flex-col group">
                  <div className={`flex-1 bg-white dark:bg-slate-900 rounded-[2rem] border-2 transition-all duration-300 flex flex-col p-8 ${isPro ? 'border-primary-500 shadow-xl shadow-primary-500/5' : 'border-slate-100 dark:border-slate-800'
                    }`}>
                    {/* Minimalist Header */}
