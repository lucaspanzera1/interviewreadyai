...
const UserQuizPage: React.FC = () => {
    ...
    if (currentQuestion < quiz.questions.length - 1) {
      ...
      if (quizId) {
        ...
        try {
          ...
          // For user quizzes, just log errors but still show results
        }
      }
  ...
  const restartQuiz = async () => {
    try {
      // No free quiz limit check for user-created quizzes
      // Just record access for the restart
      if (quizId) {
        await apiClient.recordQuizAccess(quizId).catch(console.error);
      }
